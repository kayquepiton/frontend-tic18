...
export class FormUsuarioComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private dialog: MatDialog) {
    ...
    // Subscrever ao statusChanges de cada controle do formulário
    Object.keys(this.userForm.controls).forEach(key => {
      const control = this.userForm.get(key);
      if (control) { // Verifica se control não é nulo
        control.statusChanges.subscribe(status => {
          console.log(`Status do controle ${key}: ${status}`);
        });
      }
    });
  }
  ...
  isPasswordInvalid(validationType: string): boolean {
    const passwordControl = this.userForm.get('password');
    if (passwordControl && passwordControl.errors) {
      if (passwordControl.errors['required']) {
        return false; // Se o campo senha estiver vazio, não mostramos as mensagens de validação
      }
      return passwordControl.errors[validationType];
    }
    return false;
  }
